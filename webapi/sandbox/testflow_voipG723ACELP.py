...
flowList = [
			# Voice Flow
				["G.723.1-ACELP (5.3 kbps)", 			1, 		 "RTP",  	"Voice",		"None"],
...
try:
	...
	# Note: the statistic names should be identical to those that appear in the results CSV
	results = ixchariotApi.getFlowLevelResults(session, ["Avg Mos", "Avg R-value"], "FlowGroup 1", "G.723.1ACELP (64 kbps)", "RTP")
	
	print "Flow Level Results for G.723.1ACELP (64 kbps) from FlowGroup 1:\n"
	for res in results:
		print res
		print ""
	...
	results = ixchariotApi.getUserLevelResultsFromFlow(session, ["Avg Mos"], "FlowGroup 1", "G.723.1ACELP (64 kbps)", "RTP", 1)
	
	print "User Level Results for first user in flow G.723.1ACELP (64 kbps) (RTP), FlowGroup 1:\n"
	for res in results:
		print res
		print ""
