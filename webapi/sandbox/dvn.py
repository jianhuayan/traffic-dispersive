...
def db_table(dbname,tablename):
    try:
        cnx = mysql.connector.connect(user='root', password='D1spers1ve', host=const.RestApiIP, database=dbname)
        # cursor = cnx.cursor(buffered=True)
        cursor = cnx.cursor()
        print "we have access to the DB\n"
        column = []
        print column
        # query =("SELECT * FROM tablename")
        query =("SELECT * FROM JtBf_set")
        columm2 = cursor.execute(query)
        # return columm2
        for (TestNum, Delay, Jitter, Packetloss) in cursor:
            print ("{}, {}, {}".format(Delay, Jitter, Packetloss))
            #print ("{}, {}, {}".format(Delay[0], Jitter[0], Packetloss[4]))
            # dvn.impairments("172.16.20.1", "p1p1", Delay, Jitter, Packetloss)
            # break
        cursor.close()
        cnx.close()
...
#def dbquery():
def dbquerycolumn(column_name, table_name):
        cnx = mysql.connector.connect(user='root', password='D1spers1ve', host=const.RestApiIP, database='AutoDB')
        cursor = cnx.cursor()
        query = ("SELECT column_name FROM table_name")
        column = cursor.execute(query)
        column.pop(0)
        print column
